...
/*
 * proc_create_user_gui — Crear el proceso GUI en Ring 3.
 *
 * El código del GUI está físicamente en el espacio del kernel (fue compilado
 * junto con él). Para que corra en Ring 3 necesitamos:
 *
 *  1. Crear un page directory propio para el proceso.
 *  2. Mapear el rango de código del kernel que contiene GuiMainLoop() con
 *     el flag PTE_USER, para que la CPU en Ring 3 pueda leerlo.
 *     Mapeamos 0x00000000..0x07FFFFFF (igual que el kernel) con PTE_USER.
 *     El código es de solo lectura (no PTE_WRITABLE para páginas de texto).
 *  3. Mapear el rango de datos del kernel con PTE_WRITABLE | PTE_USER
 *     (el proceso necesita leer/escribir sus propias variables estáticas
 *     como s_cursor_bg, s_saved_x, etc.).
 *  4. Crear stack de usuario separado (Ring 3 stack).
 *  5. Construir el frame de IRET con CS=0x1B, SS=0x23, EFLAGS con IF=1.
 *
 * NOTA DE SEGURIDAD: Mapeamos TODO el espacio del kernel con PTE_USER.
 * Esto NO es un OS de producción — en un sistema real el código del kernel
 * estaría en la mitad alta (>= 0x80000000) separada del espacio de usuario.
 * Para este proyecto educativo, el tradeoff es aceptable: el proceso GUI
 * corre en Ring 3 y no puede ejecutar instrucciones privilegiadas, aunque
 * puede leer la memoria del kernel mapeada. En v0.5 moveremos el kernel a
 * la mitad alta para una separación real.
 */
process_t* proc_create_user_gui(const char* name, void (*entry_point)(void))
{
    process_t* proc = alloc_process();
    if (!proc) return NULL;
    ...
    /* Crear page directory propio */
    proc->page_dir = vmm_create_directory();
    if (!proc->page_dir) { proc->active = 0; return NULL; }

    /*
     * Mapear el espacio del kernel en el page directory del proceso
     * con PTE_USER para que Ring 3 pueda ejecutar el código.
     *
     * Mapeamos 0x00000000 → 0x07FFFFFF (128 MB, mismo rango que vmm_init).
     * Esto incluye:
     *   - Código del kernel y del GUI (texto del ejecutable)
     *   - Datos globales del GUI (s_cursor_bg, WINDOW structs, etc.)
     *   - VGA framebuffer 0xA0000 (dentro del rango)
     */
    {
        uint32_t addr;
        for (addr = 0; addr < 0x08000000; addr += 4096) {
            vmm_map_page(proc->page_dir, addr, addr,
                         PTE_PRESENT | PTE_WRITABLE | PTE_USER);
        }
    }

    /* Mapear stack de usuario en 0x7FFF0000..0x7FFFFFFF */
    {
        uint32_t stack_virt  = USER_STACK_TOP - USER_STACK_SIZE;
        uint32_t stack_pages = USER_STACK_SIZE / PAGE_SIZE;
        uint32_t p;
        for (p = 0; p < stack_pages; p++) {
    ...
    /* Stack del KERNEL para este thread (Ring 0 al manejar syscalls/IRQs) */
    uint32_t kstack = pmm_alloc_frame();
    t->kernel_stack_base = kstack;
    t->kernel_stack_top  = kstack + PAGE_SIZE;

    /* Construir el frame de IRET para Ring 3:
     * entry_point es una dirección virtual del kernel, pero como mapeamos
     * el kernel con PTE_USER, esa dirección también es válida en Ring 3. */
